...
if (isset($_GET['set_id'])) {
    ...
    if ($filter_set) {
        ...
        $query = "
            ...
            FROM laws
            INNER JOIN law_types ON laws.law_types_id = law_types.id
        ";
        $params = [];
        $types = "";

        // Fetch type IDs linked to the filter set
        $sql_type_ids = "SELECT law_types FROM filter_set_items WHERE filter_set_id = ?";
...
<body>
    <button class='btnGoBack'><a href="./main.php">НА ГЛАВНЫЙ ЭКРАН</a></button>
    <h1>Рекомендации</h1>
        ...
        <?php if ($result_blocks->num_rows > 0): ?>
            <ul>
                <?php while ($block = $result_blocks->fetch_assoc()): ?>
                    <li>
                        <a href="recomends.php?set_id=<?= $block['id'] ?>" onClick="btnGoBack()">
